...
class ScriptedDocumentProvider:
    def __init__(
        self,
        *,
...
def override_document_provider(test_app: FastAPI, provider: ScriptedDocumentProvider) -> None:
...
def test_single_user_and_wishlist_crud_are_android_ready(
    client: TestClient,
    db_session: Session,
) -> None:
    user = db_session.get(User, SINGLE_USER_ID)
    assert user is not None
    assert user.name == "Single User"
    assert client.get("/api/v1/me/wishlist").json() == {"items": []}

    first = client.post("/api/v1/me/wishlist/diptyque_leau_papier_100_001")
    duplicate = client.post("/api/v1/me/wishlist/diptyque_leau_papier_100_001")

    assert first.status_code == 200
    assert duplicate.status_code == 200
    assert first.json()["productId"] == "diptyque_leau_papier_100_001"
    assert db_session.scalar(select(func.count()).select_from(WishlistItem)) == 1
    assert client.get("/api/v1/me/wishlist").json()["items"][0]["productId"] == (
    ...
    assert client.delete("/api/v1/me/wishlist/diptyque_leau_papier_100_001").status_code == 204
    assert client.delete("/api/v1/me/wishlist/diptyque_leau_papier_100_001").status_code == 204
    assert client.get("/api/v1/me/wishlist").json() == {"items": []}
...
def test_receipt_analysis_saves_items_and_only_exact_product_mapping(
    ...
    provider = ScriptedDocumentProvider(
        receipts=[
            ReceiptExtraction(
                store_name="Reference Department Store",
    ...
    assert matched["product"]["productId"] == "diptyque_leau_papier_100_001"
...
def test_receipt_provider_failure_is_503_and_does_not_save(
    ...
    override_document_provider(
        test_app,
        ScriptedDocumentProvider(error=DocumentExtractionProviderError("invalid output")),
    )
...
def test_receipt_rejects_invalid_image(
    ...
    override_document_provider(test_app, ScriptedDocumentProvider())
...
def test_flight_analysis_and_my_page_use_latest_flight(
    ...
    assert my_page.json()["user"] == {"id": 1, "name": "Single User"}
...
def _seed_recommendation_history(db: Session) -> None:
    ...
    db.add(
        SessionProduct(
            ...
            product_id=products["anillo_fragrance_of_life_10_001"].id,
    ...
    db.add(
        WishlistItem(
            user_id=SINGLE_USER_ID,
            product_id=products["diptyque_leau_papier_100_001"].id,
        )
    )
    receipt = Receipt(
        user_id=SINGLE_USER_ID,
    ...
    receipt.items.append(
        ReceiptItem(
            product_name=products["dashu_aqua_dive_50_001"].name,
            product=products["dashu_aqua_dive_50_001"],
            quantity=1,
            price=25_000,
        )
    )
...
def test_recommendation_context_contains_all_history_and_only_db_candidates(
    ...
    candidate_product = db_session.scalar(
        select(Product).where(Product.product_id == "mcm_cosmic_star_edp_75_001")
    )
    ...
    assert context.wishlist_product_ids == ["diptyque_leau_papier_100_001"]
    assert context.viewed_products[0].product_id == "anillo_fragrance_of_life_10_001"
    assert context.viewed_products[0].observation_count == 3
    assert context.viewed_products[0].store_ids == [history_store_id]
    assert context.purchased_product_ids == ["dashu_aqua_dive_50_001"]
    matched_purchase = next(
        item for item in context.purchased_products if item.product_id == "dashu_aqua_dive_50_001"
    )
    ...
    assert unmatched_purchase.store_name == "Reference Store"
    ...
    assert "dashu_aqua_dive_50_001" not in candidate_ids
...
def test_b5_api_e2e_flow_with_scripted_openai_providers(
    client: TestClient,
    test_app: FastAPI,
    db_session: Session,
) -> None:
    receipt_product = db_session.scalar(
        select(Product).where(Product.product_id == "dashu_aqua_dive_50_001")
    )
    assert receipt_product is not None
    document_provider = ScriptedDocumentProvider(
        receipts=[
            ReceiptExtraction(
                store_name="E2E Reference Store",
                purchased_at=datetime.fromisoformat("2026-08-19T14:30:00+09:00"),
                currency="KRW",
    ...
    assert db_session.get(User, SINGLE_USER_ID) is not None
    ...
    assert client.post("/api/v1/me/wishlist/diptyque_leau_papier_100_001").status_code == 200
    assert client.get("/api/v1/me/wishlist").json()["items"][0]["productId"] == (
        "diptyque_leau_papier_100_001"
    )
    assert (
    ...
    assert my_page_response.json()["wishlist"][0]["productId"] == "diptyque_leau_papier_100_001"
